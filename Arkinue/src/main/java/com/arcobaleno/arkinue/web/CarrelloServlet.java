...
public class CarrelloServlet extends HttpServlet
{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException 
	{
		try 
		{
			...
			{
				String articoloNelCarrello = req.getParameter(Costanti.ID);
				Integer articoloId= Integer.parseInt(articoloNelCarrello);

				if(bl.carrelloServlet(utenteloggato, articoloId)!= null) 
				{
					resp.sendRedirect(req.getContextPath() + "/visualizzaCarrello");
				}
				else 
				{
					req.setAttribute(Costanti.ERRORE_MSG, "L' oggetto non esiste più nel catalogo");
